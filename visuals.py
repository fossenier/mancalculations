...
class Visuals:
    def draw_mancala_board(self, stdscr, vertical_offset, horizontal_offset):
        """
        ...
        self.draw_store(
            stdscr, P2, vertical_offset + HEIGHT_IDENTIFIERS, horizontal_offset
        )
        ...
        for pit in range(PLAYER_PIT_COUNT):
            ...
            self.draw_pit(
                stdscr,
                P2,
                vertical_offset + HEIGHT_IDENTIFIERS,
            ...
            self.draw_pit(
                stdscr,
                P1,
                vertical_offset + HEIGHT_IDENTIFIERS + EFFECTIVE_HEIGHT_PIT,
    ...
    def draw_message(
        self, stdscr, vertical_offset, horizontal_offset, message, wait_time
    ):
        """
        ...
        """
        self.draw_blank(stdscr, vertical_offset, horizontal_offset, 1)
        self.draw_text(stdscr, vertical_offset, horizontal_offset, message)
        time.sleep(wait_time)
        self.draw_blank(stdscr, vertical_offset, horizontal_offset, 1)

    def draw_pit(
        self, stdscr, player_turn, vertical_offset, horizontal_offset, pit_selection
    ):
        """
        ...
        pit_contents = str(self.p_pits[player_turn][pit_selection])
    ...
    def draw_store(self, stdscr, player_turn, vertical_offset, horizontal_offset):
        """
        ...
        Args:
            ...
            `horizontal_offset` (`integer`): Chosen Curses main window horizontal offset.\n
        """
        pit_contents = str(self.p_store[player_turn])
    ...
    def draw_game_over_animation(stdscr, player1_score, player2_score):
        ...
        Args:
            `stdscr` (`stdscr`): Curses main window.\n
            `player1_score` (`integer`): Player 1's score.\n
            `player2_score` (`integer`): Player 2's score.
        """
        # clear the screen
        stdscr.clear()
    ...
    def draw_pit_identifier(stdscr, vertical_offset, horizontal_offset, pit_selection):
        """
        Draws an identifying number of a pit for the user.
        ...
        """
        stdscr.addstr(
            vertical_offset, horizontal_offset, f"{pit_selection:^{WIDTH_PIT}}"
        )
